...
/// This node's application layer: the group protocol.
///
/// It lives in `ac-groups`, which cannot add a field to `AcBehaviour` from outside its own
/// crate — so the binary supplies it through the app slot instead. That is the whole reason
/// the slot exists, and why `ac-net` names no group type.
pub type App = ac_groups::wire::Behaviour;

/// A request-response behaviour is not const-constructible, so this is a function where the
/// `dummy::Behaviour` it replaced was a `const`.
pub fn app() -> App {
    ac_groups::wire::behaviour()
}
    ...
    loop {
        tokio::select! {
            _ = tick.tick() => {
                ...
                dispatch_admission(&mut swarm, actions, &mut groups);
            ...
            event = swarm.select_next_some() => {
                ...
                match event {
                    SwarmEvent::Behaviour(AcBehaviourEvent::PeerAttest(event)) => {
                        on_peer_attest(&mut swarm, &mut admission, &mut groups, event);
                    }
                    SwarmEvent::Behaviour(AcBehaviourEvent::Attest(event)) => {
                        on_renewal(&mut swarm, &mut admission, &mut groups, event);
                    }
                    SwarmEvent::Behaviour(AcBehaviourEvent::App(event)) => {
...
fn on_peer_attest(
    ...
    dispatch_admission(swarm, actions, groups);
}
